...
$(document).ready(function () {
  ...
  randomMealButton.on("click", function () {
    ...
    setTimeout(function () {
      $("html, body").animate(
        { scrollTop: $(".display-single").offset().top },
        1000
      );
    }, 300);
  });
...
function getMealWithIngredient(ingredient) {
  ...
  $.ajax({
    ...
    if (response.meals === null) {
      showAlert(
        "There were no meals found with that search term. Please try again."
      );
    } else {
...
function displaySingleMeal(mealToDisplay) {
  ...
  for (key in mealToDisplay.meals[0]) {
    if (key.includes("strIngredient")) {
      if (
        mealToDisplay.meals[0][key] !== null &&
        mealToDisplay.meals[0][key] !== ""
      ) {
